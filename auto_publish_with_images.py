...
<footer>
  ...
  <div style="display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; margin-bottom: 16px;">
    ...
    <a href="mailto:eunhaebleu@naver.com" style="font-size: 13px; color: #666; text-decoration: none;">문의</a>
  </div>
  <p class="footer-coupang-disclaimer" style="font-size: 11px; color: #999; margin: 0 0 16px 0; line-height: 1.4;"><span style="display:none;">이 사이트는 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.</span></p>
  <div class="footer-branding">Crossero Puzzle <strong>Engine by PuzDuk</strong></div>
...
.board-search-wrap {{ margin-bottom: 18px; }}
.board-search {{ width: 100%; box-sizing: border-box; padding: 12px 16px; border: 1px solid #ddd; border-radius: 8px; font-size: 15px; font-family: inherit; }}
.board-search:focus {{ outline: none; border-color: #0073e6; }}
.board-empty {{ display: none; text-align: center; color: #888; padding: 30px 0; font-size: 14px; }}
.footer-divider {{ height: 1px; background: linear-gradient(to right, transparent, #ccc, transparent); margin-bottom: 30px; }}
.footer-branding {{ font-size: 14px; color: #888; margin-bottom: 10px; text-align: center; }}
...
<div class="board-search-wrap">
  <input type="text" id="boardSearch" class="board-search" placeholder="퍼즐 제목으로 검색 (예: 창세기, 이사야, 성령)" autocomplete="off">
</div>
<p class="board-empty" id="boardEmpty">검색 결과가 없습니다.</p>
<ul class="board-list">
{list_rows}
</ul>
</div>
...
(function(){{
  ...
  var items = document.querySelectorAll('.board-list > li');
  if (!input) return;
  input.addEventListener('input', function(){{
    var q = input.value.trim().toLowerCase();
    var visible = 0;
    items.forEach(function(li){{
      var title = li.querySelector('.board-title').textContent.toLowerCase();
